...
# TRAIN TEST SUMMARY
# =========================
# NOTE: Lưu thông tin train/test để dashboard trực quan hóa pipeline ML.

train_test_summary = pd.DataFrame([{
    "train_rows": len(X_train),
    "test_rows": len(X_test),
    "total_rows": len(df),
    "train_percent": round(len(X_train) / len(df) * 100, 2),
    "test_percent": round(len(X_test) / len(df) * 100, 2),
    "train_default_rate": round(y_train.mean() * 100, 2),
    "test_default_rate": round(y_test.mean() * 100, 2)
}])

train_test_summary.to_csv(
    os.path.join(REPORT_DIR, "train_test_summary.csv"),
    index=False,
    encoding="utf-8-sig"
)

print("Saved: reports/train_test_summary.csv")
